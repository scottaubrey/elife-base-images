...
build-php-7.4:
	@$(MAKE) -f $(PHP_MAKEFILE) REPO_PREFIX=$(REPO_PREFIX) VERSION=7.4 build
build-php-8.0:
	@$(MAKE) -f $(PHP_MAKEFILE) REPO_PREFIX=$(REPO_PREFIX) VERSION=8.0 build

# run the baseline tests that the images run
test-php: test-php-7.4 test-php-8.0
test-php-7.4:
	@$(MAKE) -f $(PHP_MAKEFILE) REPO_PREFIX=$(REPO_PREFIX) VERSION=7.4 test
test-php-8.0:
	@$(MAKE) -f $(PHP_MAKEFILE) REPO_PREFIX=$(REPO_PREFIX) VERSION=8.0 test
